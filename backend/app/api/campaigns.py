...
from app.services.webhook import notify_campaign_complete
...
async def _run_campaign_pipeline(campaign_id: str):
    ...
    try:
        ...
        await notify_campaign_complete(
            title=data["title"],
            viral_score=report.get("viral_score"),
            summary=report.get("summary"),
            campaign_id=campaign_id,
...
async def scrape_context_urls(
    campaign_id: str,
    req: ScrapeRequest,
    db: AsyncSession = Depends(get_db),
):
    """Scrape URLs and append extracted text as campaign context."""
    campaign = await db.get(Campaign, campaign_id)
    if not campaign:
        raise HTTPException(status_code=404, detail="Campaign not found")
    ...
    for url in req.urls:
        try:
            ...
            # Append to context
            label = data["title"] or url
            chunk = f"\n\n--- Scraped: {label} ---\n\n{data['text']}"
            if campaign.context_text:
                campaign.context_text += chunk
            else:
                campaign.context_text = chunk.strip()
            results.append({"url": url, "title": data["title"], "chars": data["chars"], "status": "ok"})
        except Exception as e:
            results.append({"url": url, "title": "", "chars": 0, "status": f"error: {str(e)[:100]}"})

    await db.commit()

    return {"scraped": len([r for r in results if r["status"] == "ok"]), "results": results}
