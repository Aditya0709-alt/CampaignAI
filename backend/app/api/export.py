...
async def export_csv(simulation_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=CampaignAI-{simulation_id[:8]}.csv"},
    )
...
async def export_json(simulation_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=CampaignAI-{simulation_id[:8]}.json"},
    )
